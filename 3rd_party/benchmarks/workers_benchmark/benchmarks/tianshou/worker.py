"""Tianshou worker benchmark -- PPO through tianshou_worker.launcher.

Calls the launcher module directly (same as TianshouWorkerRuntime does),
bypassing the runtime's dependency on gym_gui.config.paths.
"""
...
def run_worker_benchmark(config) -> BenchmarkResult:
    """Run Tianshou PPO via the worker launcher subprocess."""
    ...
    # Write config JSON like the runtime does
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config_dict = {
            "run_id": f"bench_tianshou_worker_{config.seed}",
            "algo": "ppo",
            ...
            "extras": {
                ...
                "epoch": 1,
                "step_per_epoch": config.total_timesteps,
        ...
        cmd = [
            sys.executable, "-m", "tianshou_worker.launcher",
