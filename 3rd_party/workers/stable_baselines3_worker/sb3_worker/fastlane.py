"""FastLane telemetry helpers for the SB3 worker.

Thin module that reuses CleanRL's FastLaneTelemetryWrapper to provide
real-time frame streaming for Stable-Baselines3 training runs.
"""
...
def _resolve_config() -> _FastLaneConfig:
    ...
    try:
        total_envs = max(1, int(os.getenv("SB3_NUM_ENVS", os.getenv("NUM_ENVS", "1"))))
    except ValueError:
        total_envs = 1

    slot = max(0, min(slot, total_envs - 1))

    run_id = os.getenv("SB3_RUN_ID") or os.getenv("RUN_ID") or "sb3-run"
    agent_id = os.getenv("SB3_AGENT_ID") or os.getenv("AGENT_ID") or "sb3-agent"
    worker_id = (
        os.getenv("WORKER_ID")
        or os.getenv("SB3_WORKER_ID")
        or "sb3-worker"
    )

    try:
        seed = int(os.getenv("SB3_SEED", os.getenv("SEED", "0")))
    except ValueError:
        seed = 0
...
def maybe_wrap_env(env: Any) -> Any:
    ...
    try:
        ...
        _LOGGER.debug("[SB3-FASTLANE] Failed to wrap env at slot %d", slot_index)
        return env
