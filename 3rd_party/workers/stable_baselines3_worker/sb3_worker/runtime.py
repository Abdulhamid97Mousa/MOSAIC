"""Runtime orchestration helpers for launching Stable-Baselines3 algorithms."""
...
try:
    ...
    from gym_gui.logging_config.log_constants import (
        LOG_WORKER_SB3_RUNTIME_STARTED,
        LOG_WORKER_SB3_RUNTIME_COMPLETED,
        LOG_WORKER_SB3_RUNTIME_FAILED,
        LOG_WORKER_SB3_SUBPROCESS_STDOUT,
        LOG_WORKER_SB3_SUBPROCESS_STDERR,
    )
...
class SB3WorkerRuntime:
    """Orchestrates Stable-Baselines3 training jobs."""

    def __init__(self, work_dir: Path | str) -> None:
        self.work_dir = Path(work_dir)
        self.work_dir.mkdir(parents=True, exist_ok=True)

    def execute(self, config: SB3WorkerConfig) -> None:
        """Execute a Stable-Baselines3 training run based on configuration.

        Args:
            config: SB3WorkerConfig instance
        """
        if _HAS_LOG_CONSTANTS:
            ...
            LOGGER.info(
                "SB3 worker runtime started: run_id=%s algo=%s env_id=%s",
        ...
        try:
            ...
            if return_code != 0:
                raise RuntimeError(f"SB3 worker process failed with exit code {return_code}")
            ...
            if _HAS_LOG_CONSTANTS:
                ...
                LOGGER.info("SB3 worker runtime completed: run_id=%s", config.run_id)
            ...
            if _HAS_LOG_CONSTANTS:
                ...
                LOGGER.exception("SB3 worker runtime failed: run_id=%s", config.run_id)
            raise
