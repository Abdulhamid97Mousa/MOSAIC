"""Configuration parsing helpers for the SB3 worker shim."""
...
try:
    ...
    WorkerConfigProtocol = None


@dataclass
class SB3WorkerConfig:
    """Structured configuration for launching a Stable-Baselines3 worker run.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("sb3_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("sb3_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("sb3_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("sb3_worker config missing required field 'total_timesteps'")
        ...
        if WorkerConfigProtocol is not None:
            assert isinstance(self, WorkerConfigProtocol), (
                "SB3WorkerConfig must implement WorkerConfig protocol"
            )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "SB3WorkerConfig":
...
def load_worker_config(path: Path) -> SB3WorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return SB3WorkerConfig.from_dict(worker_config)

    return SB3WorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = SB3WorkerConfig
