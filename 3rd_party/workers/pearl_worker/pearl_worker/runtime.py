"""Runtime orchestration helpers for launching Pearl algorithms."""
...
try:
    ...
    from gym_gui.logging_config.log_constants import (
        LOG_WORKER_PEARL_RUNTIME_STARTED,
        LOG_WORKER_PEARL_RUNTIME_COMPLETED,
        LOG_WORKER_PEARL_RUNTIME_FAILED,
        LOG_WORKER_PEARL_SUBPROCESS_STDOUT,
        LOG_WORKER_PEARL_SUBPROCESS_STDERR,
    )
    _HAS_GYM_GUI = True
except ImportError:
    _HAS_GYM_GUI = False
...
class PearlWorkerRuntime:
    """Orchestrates Pearl training jobs."""

    def __init__(self, work_dir: Path | str) -> None:
        self.work_dir = Path(work_dir)
        self.work_dir.mkdir(parents=True, exist_ok=True)

    def execute(self, config: PearlWorkerConfig) -> None:
        """Execute a Pearl training run based on configuration.

        Args:
            config: PearlWorkerConfig instance
        """
        if _HAS_GYM_GUI:
            log_constant(
                LOGGER,
            ...
            # Ensure work directory for the run exists
            run_dir = VAR_TRAINER_DIR / "runs" / config.run_id
            run_dir.mkdir(parents=True, exist_ok=True)
        else:
            LOGGER.info(
                "Pearl worker runtime started: run_id=%s algo=%s env_id=%s",
                config.run_id, config.algo, config.env_id,
            )
            run_dir = self.work_dir / "runs" / config.run_id
            run_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            if return_code != 0:
                raise RuntimeError(f"Pearl worker process failed with exit code {return_code}")
            ...
            if _HAS_GYM_GUI:
                ...
                LOGGER.info("Pearl worker runtime completed: run_id=%s", config.run_id)
            ...
            if _HAS_GYM_GUI:
                ...
                LOGGER.exception("Pearl worker runtime failed: run_id=%s", config.run_id)
            raise
