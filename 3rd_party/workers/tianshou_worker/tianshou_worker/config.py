"""Configuration parsing helpers for the Tianshou worker shim."""
...
class TianshouWorkerConfig:
    """Structured configuration for launching a Tianshou worker run.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("tianshou_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("tianshou_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("tianshou_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("tianshou_worker config missing required field 'total_timesteps'")

        # Assert protocol compliance (structural typing check)
        assert isinstance(self, WorkerConfigProtocol), (
            "TianshouWorkerConfig must implement WorkerConfig protocol"
        )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "TianshouWorkerConfig":
...
def load_worker_config(path: Path) -> TianshouWorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return TianshouWorkerConfig.from_dict(worker_config)
        
    return TianshouWorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = TianshouWorkerConfig
