"""FastLane telemetry helpers for the Tianshou worker.

Thin module that reuses CleanRL's FastLaneTelemetryWrapper to provide
real-time frame streaming for Tianshou training runs.
"""
...
def _resolve_config() -> _FastLaneConfig:
    ...
    try:
        total_envs = max(1, int(os.getenv("TIANSHOU_NUM_ENVS", os.getenv("NUM_ENVS", "1"))))
    except ValueError:
        total_envs = 1

    slot = max(0, min(slot, total_envs - 1))

    run_id = os.getenv("TIANSHOU_RUN_ID") or os.getenv("RUN_ID") or "tianshou-run"
    agent_id = os.getenv("TIANSHOU_AGENT_ID") or os.getenv("AGENT_ID") or "tianshou-agent"
    worker_id = (
        os.getenv("WORKER_ID")
        or os.getenv("TIANSHOU_WORKER_ID")
        or "tianshou-worker"
    )

    try:
        seed = int(os.getenv("TIANSHOU_SEED", os.getenv("SEED", "0")))
    except ValueError:
        seed = 0
...
def maybe_wrap_env(env: Any) -> Any:
    ...
    try:
        ...
        _LOGGER.debug("[TIANSHOU-FASTLANE] Failed to wrap env at slot %d", slot_index)
        return env
