"""FastLane telemetry helpers for the SBX worker.

Thin module that reuses CleanRL's FastLaneTelemetryWrapper to provide
real-time frame streaming for Stable-Baselines3/SBX training runs.
"""
...
def _resolve_config() -> _FastLaneConfig:
    ...
    try:
        total_envs = max(1, int(os.getenv("SBX_NUM_ENVS", os.getenv("NUM_ENVS", "1"))))
    except ValueError:
        total_envs = 1

    slot = max(0, min(slot, total_envs - 1))

    run_id = os.getenv("SBX_RUN_ID") or os.getenv("RUN_ID") or "sbx-run"
    agent_id = os.getenv("SBX_AGENT_ID") or os.getenv("AGENT_ID") or "sbx-agent"
    worker_id = (
        os.getenv("WORKER_ID")
        or os.getenv("SBX_WORKER_ID")
        or os.getenv("SBX_AGENT_ID")
        or "sbx-worker"
    )

    try:
        seed = int(os.getenv("SBX_SEED", os.getenv("SEED", "0")))
    except ValueError:
        seed = 0
...
def maybe_wrap_env(env: Any) -> Any:
    ...
    Returns *env* unchanged when FastLane is disabled, the wrapper class
    could not be imported, or the video mode is OFF.
    """
    ...
    try:
        ...
        _LOGGER.debug("[SBX-FASTLANE] Failed to wrap env at slot %d", slot_index)
        return env


def reload_fastlane_config() -> _FastLaneConfig:
    """Recompute FastLane config from current environment variables."""
    global _CONFIG
    _CONFIG = _resolve_config()
    return _CONFIG


__all__ = ["is_fastlane_enabled", "maybe_wrap_env", "reload_fastlane_config"]
