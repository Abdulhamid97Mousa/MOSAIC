"""Configuration parsing helpers for the SBX worker shim."""
...
@dataclass
class SBXWorkerConfig:
    """Structured configuration for launching an SBX worker run.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("sbx_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("sbx_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("sbx_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("sbx_worker config missing required field 'total_timesteps'")
        ...
        if WorkerConfigProtocol is not None:
            assert isinstance(self, WorkerConfigProtocol), (
                "SBXWorkerConfig must implement WorkerConfig protocol"
            )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "SBXWorkerConfig":
...
def load_worker_config(path: Path) -> SBXWorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return SBXWorkerConfig.from_dict(worker_config)

    return SBXWorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = SBXWorkerConfig
