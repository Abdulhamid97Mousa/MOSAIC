...
def main() -> int:
    """Main entry point for the SBX worker CLI."""
    parser = argparse.ArgumentParser(description="SBX (Stable-Baselines Jax) Worker CLI")
    parser.add_argument(
        "--config",
        type=Path,
        required=True,
    ...
    try:
        ...
        runtime = SBXWorkerRuntime(work_dir=args.work_dir)
