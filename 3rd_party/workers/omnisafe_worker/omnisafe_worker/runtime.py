"""Runtime orchestration helpers for launching OmniSafe algorithms."""
...
from .config import OmniSafeWorkerConfig

LOGGER = logging.getLogger(__name__)


class OmniSafeWorkerRuntime:
    """Orchestrates OmniSafe training jobs."""

    def __init__(self, work_dir: Path | str) -> None:
        self.work_dir = Path(work_dir)
        self.work_dir.mkdir(parents=True, exist_ok=True)

    def execute(self, config: OmniSafeWorkerConfig) -> None:
        """Execute an OmniSafe training run based on configuration.

        Args:
            config: OmniSafeWorkerConfig instance
        """
        if _HAS_GUI:
            log_constant(
                LOGGER,
            ...
            LOGGER.info(
                "OmniSafe runtime started: run_id=%s algo=%s env_id=%s",
        ...
        # Configure FastLane
        if _HAS_GUI and config.extras.get("fastlane_enabled"):
            try:
        ...
        try:
            ...
            if return_code != 0:
                raise RuntimeError(f"OmniSafe worker process failed with exit code {return_code}")
            ...
            if _HAS_GUI:
                ...
                LOGGER.info("OmniSafe runtime completed: run_id=%s", config.run_id)
            ...
            if _HAS_GUI:
                ...
                LOGGER.exception("OmniSafe runtime failed: run_id=%s", config.run_id)
            raise
