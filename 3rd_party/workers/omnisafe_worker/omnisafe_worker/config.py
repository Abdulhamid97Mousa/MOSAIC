"""Configuration parsing helpers for the OmniSafe worker shim."""
...
# Map standard Gymnasium IDs to Safety-Gymnasium equivalents
ENV_MAP: dict[str, str] = {
    "CartPole-v1": "SafetyCartPole-v0",
    "Pendulum-v1": "SafetyPointGoal0-v0",
}


@dataclass
class OmniSafeWorkerConfig:
    """Structured configuration for launching an OmniSafe worker run.
    ...
    @property
    def safe_env_id(self) -> str:
        """Map a standard gym env_id to its Safety-Gymnasium equivalent.

        If the env_id is already a Safety-Gymnasium ID (or not in the map),
        it is returned unchanged.
        """
        return ENV_MAP.get(self.env_id, self.env_id)

    def __post_init__(self) -> None:
        """Validate required fields on initialization."""
        if not self.run_id:
            raise ValueError("omnisafe_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("omnisafe_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("omnisafe_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("omnisafe_worker config missing required field 'total_timesteps'")
        ...
        if WorkerConfigProtocol is not None:
            assert isinstance(self, WorkerConfigProtocol), (
                "OmniSafeWorkerConfig must implement WorkerConfig protocol"
            )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "OmniSafeWorkerConfig":
...
def load_worker_config(path: Path) -> OmniSafeWorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return OmniSafeWorkerConfig.from_dict(worker_config)

    return OmniSafeWorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = OmniSafeWorkerConfig
