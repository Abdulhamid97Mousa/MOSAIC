"""JAX-native Basketball environment — JaxMARL MultiAgentEnv compatible.

Grid: 19 × 11
  - Walls on all borders
  - Green hoop: x=1,  y=5  (single cell)
  - Blue  hoop: x=17, y=5
  - Ball spawns at (9, 5)
  - Goals to win: 2

Actions (8, gym v2): 0=noop 1=left 2=right 3=forward 4=pickup 5=drop 6=toggle 7=done
...
GREEN_HOOP_X = jnp.int32(1)
BLUE_HOOP_X  = jnp.int32(17)
HOOP_Y            = jnp.int32(5)   # kept for STATIC_GRID placement
DEFAULT_GOAL_ROWS = [5]             # single hoop cell
...
_TEAMS: Dict[str, chex.Array] = {
    ...
    'G-4v0': jnp.array([0, 0, 0, 0],    dtype=jnp.int32),
...
def _make_static_grid() -> chex.Array:
    ...
    g = g.at[HOOP_Y, 1].set(5)   # green hoop
    g = g.at[HOOP_Y, 17].set(6)  # blue hoop
    return g

STATIC_GRID = _make_static_grid()

# ---------------------------------------------------------------------------
# Action logic (identical pattern to AF)
# ---------------------------------------------------------------------------

def _apply_action(state: BBState, agent_idx: int, action: chex.Array) -> BBState:
    ...
    state = jax.lax.cond(action == 3, _do_forward, lambda s: s, state)
    ...
    state = jax.lax.cond(action == 4, _do_pickup, lambda s: s, state)
    ...
    state = jax.lax.cond(action == 5, _do_drop, lambda s: s, state)
    return state

# ---------------------------------------------------------------------------
# Scoring — ball at hoop cell
# ---------------------------------------------------------------------------

def _check_scoring(state: BBState, goal_rows):
    bx = state.ball_pos[:, 0]; by = state.ball_pos[:, 1]
    in_hoop = lambda by_: jnp.any(jnp.equal(by_[:, None], goal_rows[None, :]), axis=1)
    scored_green = jnp.any((bx == BLUE_HOOP_X)  & in_hoop(by))
    scored_blue  = jnp.any((bx == GREEN_HOOP_X) & in_hoop(by))
...
def _compute_rewards(state, new_state, scored_green, scored_blue, timed_out, n_agents, goal_y: float = 5.0, ball_coef: float = 0.01, cooperative: bool = False):
    ...
    for i in range(n_agents):
        ...
        # carrying     → hoop (team 0 → BLUE_HOOP_X=17, team 1 → GREEN_HOOP_X=1)
        # not carrying → loose ball
        carrying_now = now_carrying_idx >= 0
        goal_x = jnp.where(team == 0, BLUE_HOOP_X.astype(jnp.float32),
                                       GREEN_HOOP_X.astype(jnp.float32))
...
def _obs_for_agent(state: BBState, agent_idx: int, n_agents: int, view_size: int = 7) -> chex.Array:
    ax   = state.agent_pos[agent_idx, 0]
    ay   = state.agent_pos[agent_idx, 1]
    team = state.agent_team[agent_idx]   # 0=green, 1=blue
    r = view_size // 2
    cells = []
    for dy in range(-r, r + 1):
        for dx in range(-r, r + 1):
            ...
            is_wall = (base == 1)
            # Team-relative goal encoding: the hoop a team scores IN always appears
            # as _OBJ_BLUE_GOAL (6.0) so a checkpoint trained on G-2v0 (green agents
            # score in the blue hoop) transfers correctly to B-0v2 (blue agents score
            # in the green hoop).
            #   team=0 (green): base==5 → 5.0 (own hoop),  base==6 → 6.0 (target)
            #   team=1 (blue):  base==5 → 6.0 (target),    base==6 → 5.0 (own hoop)
            goal_for_base5 = jnp.where(team == 0, _OBJ_GREEN_GOAL, _OBJ_BLUE_GOAL)
            goal_for_base6 = jnp.where(team == 0, _OBJ_BLUE_GOAL,  _OBJ_GREEN_GOAL)
            obj   = jnp.where(is_wall, _OBJ_WALL, jnp.where(base == 5, goal_for_base5, jnp.where(base == 6, goal_for_base6, _OBJ_FLOOR)))
            ...
            cells.extend([obj / 10.0, color / 5.0, st / 100.0])
...
class BasketballJAX(MultiAgentEnv):
    """JAX-native Basketball — JaxMARL compatible."""

    def __init__(self, variant: str = '3v3', max_steps: int = MAX_STEPS, goals_to_win: int = 2, view_size: int = 7, ball_coef: float = 0.01, goal_rows: list | None = None, cooperative: bool = False):
        ...
        assert goal_rows is not None, (
            "\n[BasketballJAX] goal_rows must be explicitly provided — do not rely on defaults.\n"
            "  Expected for basketball: goal_rows=[5]  (single hoop at centre)\n"
            "  In training scripts: --goal-rows 5\n"
            "  Use DEFAULT_GOAL_ROWS if you want the canonical value."
        )
        rows = list(goal_rows)
        assert len(rows) > 0, f"[BasketballJAX] goal_rows must be non-empty, got {rows!r}"
        assert len(rows) == len(set(rows)), \
            f"[BasketballJAX] goal_rows must not contain duplicates: {rows}"
        assert all(1 <= r <= HEIGHT - 2 for r in rows), \
            f"[BasketballJAX] goal_rows values must be in [1, {HEIGHT - 2}] (playable rows), got {rows}"
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[Dict, BBState]:
    ...
    @partial(jax.jit, static_argnums=(0,))
    def step_env(self, key, state: BBState, actions: Dict) -> Tuple:
    ...
    def get_obs(self, state: BBState) -> Dict:
...
VARIANT_MAP = {
    'MosaicMultiGrid-BB-G-1v0-v1':          'G-1v0',
    'MosaicMultiGrid-BB-B-0v1-v1':          'B-0v1',
    'MosaicMultiGrid-BB-1v1-IndAgObs-v1':   '1v1',
    'MosaicMultiGrid-BB-2v2-IndAgObs-v1':   '2v2',
    'MosaicMultiGrid-BB-3v3-IndAgObs-v1':   '3v3',
    'MosaicMultiGrid-BB-G-2v0-IndAgObs-v1': 'G-2v0',
    'MosaicMultiGrid-BB-G-3v0-IndAgObs-v1': 'G-3v0',
    'MosaicMultiGrid-BB-B-0v2-IndAgObs-v1': 'B-0v2',
    'MosaicMultiGrid-BB-B-0v3-IndAgObs-v1': 'B-0v3',
}

def make_bb_jax(env_id: str, **kwargs) -> BasketballJAX:
    return BasketballJAX(variant=VARIANT_MAP[env_id], **kwargs)
