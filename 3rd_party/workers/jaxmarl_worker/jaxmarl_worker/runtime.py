...
def _render_mosaic_sport_state(
    state,
    sport: str,
    view_size: int,
    jax_env=None,
) -> np.ndarray:
    """Adapt a JAX state to the interface used by the native v7 renderers.

    Geometry comes from the JAX environment modules and configured goal rows
    come from the running environment. All drawing is delegated to the same
    ``mosaic_multigrid.rendering`` functions used by operator preview.
    """
    from mosaic_multigrid.core.constants import Color, Type

    if sport == "soccer":
        from jaxmarl_worker.environments.mosaic_multigrid.S import soccer_jax as model
        from mosaic_multigrid.rendering.fifa import render_fifa as renderer

        left_x, right_x = int(model.GREEN_GOAL_X), int(model.BLUE_GOAL_X)
        default_rows = model.DEFAULT_GOAL_ROWS
        native_team_offset = 1
    elif sport == "af":
        from jaxmarl_worker.environments.mosaic_multigrid.AF import american_football_jax as model
        from mosaic_multigrid.rendering.american_football import (
            render_american_football as renderer,
        )

        left_x, right_x = int(model.GREEN_GOAL_X), int(model.BLUE_GOAL_X)
        default_rows = model.DEFAULT_GOAL_ROWS
        native_team_offset = 0
    elif sport == "bb":
        from jaxmarl_worker.environments.mosaic_multigrid.BB import basketball_jax as model
        from mosaic_multigrid.rendering.basketball import render_basketball as renderer

        left_x, right_x = int(model.GREEN_HOOP_X), int(model.BLUE_HOOP_X)
        default_rows = model.DEFAULT_GOAL_ROWS
        native_team_offset = 1
    else:
        raise ValueError(f"No native MosaicMultiGrid renderer for sport {sport!r}")

    configured_rows = getattr(jax_env, "_goal_rows", default_rows)
    goal_rows = [int(row) for row in np.asarray(configured_rows).reshape(-1)]
    width, height = int(model.WIDTH), int(model.HEIGHT)

    agent_pos = np.asarray(state.agent_pos)
    agent_dir = np.asarray(state.agent_dir)
    agent_team = np.asarray(state.agent_team)
    agent_carrying = np.asarray(state.agent_carrying)
    ball_pos = np.asarray(state.ball_pos)
    ball_carried_by = np.asarray(state.ball_carried_by)

    agents = [
        SimpleNamespace(
            index=i,
            team_index=int(agent_team[i]) + native_team_offset,
            view_size=view_size,
            state=SimpleNamespace(
                pos=np.asarray(agent_pos[i]),
                dir=int(agent_dir[i]),
                carrying=(
                    None if int(agent_carrying[i]) < 0 else SimpleNamespace()
                ),
            ),
        )
        for i in range(agent_pos.shape[0])
    ...
    loose_balls = {
        (int(ball_pos[i, 0]), int(ball_pos[i, 1]))
        for i in range(ball_pos.shape[0])
        if int(ball_carried_by[i]) < 0
    }
    ball_object = SimpleNamespace(type=Type.ball)
    endzones = {}
    if sport == "af":
        for row in goal_rows:
            endzones[(left_x, row)] = SimpleNamespace(
                type=Type.endzone, color=Color.green
            )
            endzones[(right_x, row)] = SimpleNamespace(
                type=Type.endzone, color=Color.blue
            )

    class _GridView:
        def get(self, x: int, y: int):
            if (x, y) in loose_balls:
                return ball_object
            return endzones.get((x, y))
    ...
    if sport in {"soccer", "bb"}:
        env_view.goal_pos = (
            [[left_x, row] for row in goal_rows]
            + [[right_x, row] for row in goal_rows]
        )
        env_view.goal_index = (
            [1] * len(goal_rows) + [2] * len(goal_rows)
        )

    return renderer(env_view, tile_size=32)
...
def _render_state(
    ...
    """Render a state using the environment family's authoritative renderer."""
    if sport in {"soccer", "af", "bb"}:
        return _render_mosaic_sport_state(state, sport, view_size, jax_env)
    raise ValueError(f"Unsupported rendered sport {sport!r}")




# Tile codes → [R, G, B] for coop_mining Items enum (0..6)
_COOP_MINING_TILE_COLORS = {
    0: [220, 220, 220],   # empty
    1: [127, 127, 127],   # wall
    2: [200, 200, 170],   # ore_wait
    3: [180, 180, 250],   # spawn_point
    4: [139,  69,  19],   # iron_ore
    5: [180, 180,  40],   # gold_ore
    6: [190, 190,  80],   # gold_partial
}


def _make_agent_colors(n_agents: int):
    """HSV-interpolated agent colors matching coop_mining's generate_agent_colors."""
    colors = []
    for i in range(n_agents):
...
def _state_to_grid_payload(
    ...
    """Compact grid-state payload (~3KB vs ~12MB for raw RGB)."""
    payload: Dict[str, Any] = {
        "mode":         "socialjax_grid",
        "grid":         np.array(state.grid).tolist(),
        "agent_locs":   np.array(state.agent_locs).tolist(),
        "n_agents":     n_agents,
        "inner_t":      int(inner_t),
...
def _load_mat_params(
    ...
    """Load a MAT checkpoint saved via jax.tree_util.tree_flatten(params).

    The file stores leaves only -- no tree structure.  We recover the structure
    by dummy-initialising MATActor with the same hyperparams and then calling
    jax.tree_util.tree_unflatten(tree_def, loaded_leaves).
    """
    from jaxmarl_worker.networks.mat_actor import MATActor

    data  = np.load(str(path))
    keys  = sorted(data.files, key=lambda k: int(k.split("_")[1]))
    leaves = [jnp.array(data[k]) for k in keys]

    model  = MATActor(action_dim=action_dim, n_agent=n_agents,
    ...
    template  = model.init(jax.random.PRNGKey(0), dummy_obs, dummy_sa)
    _, tree_def = jax.tree_util.tree_flatten(template)
    if len(leaves) != len(jax.tree_util.tree_leaves(template)):
        raise ValueError(
            f"MAT checkpoint leaf count mismatch: file has {len(leaves)}, "
            f"model expects {len(jax.tree_util.tree_leaves(template))}. "
            f"Check n_embd/n_head/n_block."
        )
...
def _make_env(env_id: str, view_size: int = 7, num_agents: int | None = None):
    """Construct the JAX env from a full env_id string. Returns (env, sport_key)."""
    from jaxmarl_worker.environments.soccer_jax import (
        DEFAULT_GOAL_ROWS as _S_GOAL_ROWS,
        VARIANT_MAP as _S_VARIANTS,
        make_soccer_jax,
    )
    if env_id in _S_VARIANTS:
        return make_soccer_jax(
            env_id, view_size=view_size, goal_rows=_S_GOAL_ROWS
        ), "soccer"

    from jaxmarl_worker.environments.american_football_jax import (
        DEFAULT_GOAL_ROWS as _AF_GOAL_ROWS,
        VARIANT_MAP as _AF_VARIANTS,
        make_af_jax,
    )
    if env_id in _AF_VARIANTS:
        return make_af_jax(
            env_id, view_size=view_size, goal_rows=_AF_GOAL_ROWS
        ), "af"

    from jaxmarl_worker.environments.basketball_jax import (
        DEFAULT_GOAL_ROWS as _BB_GOAL_ROWS,
        VARIANT_MAP as _BB_VARIANTS,
        make_bb_jax,
    )
    if env_id in _BB_VARIANTS:
        return make_bb_jax(
            env_id, view_size=view_size, goal_rows=_BB_GOAL_ROWS
        ), "bb"
    ...
    if sj_task in _SJ_ENVS:
        ...
        return SocialJaxGenericWrapper(sj_task, **kwargs), "socialjax"
...
class InteractiveRuntime:
    ...
    def __init__(
        ...
        self._algo        = algorithm.lower()
        self._episode_idx = 0
        self._episode_rew = 0.0
        self._step_idx    = 0

        LOGGER.info("Loading env %s (view_size=%d num_agents=%s)", env_id, view_size, num_agents)
        self._env, self._sport = _make_env(env_id, view_size, num_agents=num_agents)
        ...
        LOGGER.info("Loading actor checkpoint: %s  algo=%s", policy_path, self._algo)

        if self._algo == "mat":
            actor_params = _load_mat_params(
                Path(policy_path),
            ...
            from jaxmarl_worker.networks.mat_actor import MATActor, mat_get_actions
            self._mat_model   = MATActor(
                action_dim=self._action_dim, n_agent=self._n_agents,
                n_embd=n_embd, n_head=n_head, n_block=n_block,
            )
            self._actor_params = actor_params
            self._apply_fn     = jax.jit(self._mat_model.apply)
            self._mat_get_actions = mat_get_actions
            # Pre-warm MAT JIT
            dummy_obs = jnp.zeros((1, self._n_agents, self._obs_dim))
            dummy_sa  = jnp.zeros((1, self._n_agents, self._action_dim + 1))
            _ = self._apply_fn(self._actor_params, dummy_obs, dummy_sa)
        else:
        ...
        # Pre-compute agent colors for grid payload (socialjax/coop_mining only).
        # Other socialjax envs fall back to the existing rgb render path.
        _sj_task = env_id.split("/")[-1]
        if self._sport == "socialjax" and _sj_task == "coop_mining":
            self._agent_colors = _make_agent_colors(self._n_agents)
            self._max_inner_t = int(self._env._socialjax.num_inner_steps)
            vc = getattr(self._env._socialjax, "view_config", None)
            self._obs_view = {
                "fwd":   int(vc.forward),
                "bwd":   int(vc.backward),
                "left":  int(vc.left),
                "right": int(vc.right),
            } if vc is not None else {"fwd": 9, "bwd": 1, "left": 5, "right": 5}
        else:
    ...
    def _select_actions(self, obs_dict: Dict, key) -> Dict[str, int]:
        ...
        if self._algo == "mat":
            # MAT: stack all observations, sample autoregressively
            obs_batch = jnp.array(
                [[obs_dict[a] for a in self._agents]]
            )  # (1, n_agents, obs_dim)
            key, subkey = jax.random.split(key)
            actions_arr, _, _ = self._mat_get_actions(
                self._apply_fn, self._actor_params, obs_batch, subkey, self._action_dim
            )  # actions_arr: (1, n_agents)
            return {a: int(actions_arr[0, i]) for i, a in enumerate(self._agents)}
        # MAPPO/IPPO: independent per-agent logits
        actions = {}
    ...
    def _handle_reset(self, cmd: Dict) -> None:
        ...
        render_payload = None
        try:
            if self._sport == "socialjax" and self._agent_colors is not None:
                render_payload = _state_to_grid_payload(
                    self._state, self._n_agents,
                    int(self._state.inner_t), self._max_inner_t,
                    _COOP_MINING_TILE_COLORS, self._agent_colors,
                    obs_view=self._obs_view,
                )
            else:
                if self._sport == "socialjax":
                    img = self._env._socialjax.render(self._state)
                    rgb = np.array(img)
                    if rgb.dtype != np.uint8:
                        rgb = np.clip(rgb, 0, 255).astype(np.uint8)
                else:
                    rgb = _render_state(
                        self._state, self._sport, self._view_size, self._env
                    )
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--num-agents",  type=int, default=None, help="Override environment num_agents")
    p.add_argument("--algorithm",   default="mappo", help="Actor architecture: mappo or mat")
    p.add_argument("--n-embd",      type=int, default=128, help="MAT embedding dim")
    p.add_argument("--n-head",      type=int, default=4,   help="MAT attention heads")
    p.add_argument("--n-block",     type=int, default=2,   help="MAT transformer blocks")
