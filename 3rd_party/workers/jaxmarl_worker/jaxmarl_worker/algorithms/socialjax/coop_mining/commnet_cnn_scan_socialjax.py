...
class CommNetCNN(nn.Module):
    ...
    def __call__(self, hidden, obs_img, done):
        ...
        # C-layer on h_{t-1}: zero-init so communication starts silent and is learned
        ...
        # CNN -> project to hid_dim with tanh (matches Sukhbaatar W_e*s + comm formulation)
        cnn_out = _CNN()(o_flat)                                              # (NE*NA, 64)
        x = nn.Dense(self.hid_dim, kernel_init=orthogonal(np.sqrt(2)),
                     bias_init=constant(0.0))(cnn_out)
        x = nn.tanh(x)

        # GRU with comm injected: implements H*h + W_e*s + C*c in one pass
...
def main():
    ...
    print(f"  Mean-pool comm (CommNet, Sukhbaatar et al. 2016)  zero-init C-layer")
