"""IPPO-CNN for SocialJax coop_mining -- paper-faithful replication.

Exactly matches the SocialJax paper (Guo et al., ICLR 2026) config:
  - CNN backbone: Conv(32,5x5) -> Conv(32,3x3) -> Conv(32,3x3) -> Dense(64)
  - 4 agents, shared parameters, common reward (shared_rewards=True)
  - 100M env steps, 256 parallel envs, 1000 steps/episode
  - Saves checkpoint leaves as arr_0..arr_N for eval compatibility

Run:
  CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_PREALLOCATE=false \\
    python -m jaxmarl_worker.algorithms.socialjax.coop_mining.ippo_cnn_scan
"""
import argparse
import sys
import time
    ...
    def train(rng):
        ...
        def _update_step(runner_state, _unused):
            def _env_step(runner_state, _unused):
                ...
                # action shape: (NUM_ACTORS,) arranged as [a0_env0, a0_env1, ..., a1_env0, ...]
                # (n_agents * NUM_ENVS,) -> (n_agents, NUM_ENVS) -> list of (NUM_ENVS,)
                ...
                # reward: (NUM_ENVS, n_agents) -- transpose + reshape to (NUM_ACTORS,)
                # done:   {'__all__': (NUM_ENVS,), '0':..., ...}
                # obs:    (NUM_ENVS, n_agents, H, W, C)
                info = jax.tree.map(lambda x: x.reshape((cfg["NUM_ACTORS"],)), info)
                reward_flat = jnp.transpose(reward, (1, 0)).reshape(-1)  # (n_agents, NUM_ENVS) -> flat
                done_flat   = jnp.tile(done["__all__"], n_agents)         # broadcast to all agent slots
            ...
            _, last_val = network.apply(ts.params, last_obs_batch)  # logits unused here
...
def parse_args():
    _default_out = str(
        ...
        / "var" / "trainer" / "socialjax" / "coop_mining" / "IPPO_CNN"
    )
    ...
    p.add_argument("--num-agents",,     type=int,   default=4,    help="Paper default: 4")
    p.add_argument("--num-envs",       type=int,   default=256)
    p.add_argument("--total-timesteps",type=float, default=1e8,  help="Paper: 1e8")
...
def main():
    ...
    print(f"[IPPO-CNN] coop_mining  n_agents={args.num_agents}  "
    ...
    # Also save a readable summary of final metrics
    metrics = out["metrics"]
