"""CommNet-CNN for SocialJax coop_mining.

CommNet (Sukhbaatar et al., NeurIPS 2016) with SocialJax paper CNN backbone:
  CNN: Conv(32,5x5)->Conv(32,3x3)->Conv(32,3x3)->Dense(64)->Dense(hid_dim)
  C-layer applied to h_{t-1} (zero-init, comm starts silent)
  comm_i = (1/(N-1)) * sum_{j!=i} C(h_j^{t-1}), injected into GRU input
  PPO actor-critic, shared params across agents (IPPO-CR)
  4 agents, shared rewards, 100M env steps

Run:
  CUDA_VISIBLE_DEVICES=1 XLA_PYTHON_CLIENT_PREALLOCATE=false \\
    python -m jaxmarl_worker.algorithms.socialjax.coop_mining.commnet_cnn_scan_socialjax
"""
...
# ---------------------------------------------------------------------------
# CommNet with CNN encoder
# ---------------------------------------------------------------------------

class CommNetCNN(nn.Module):
    """Paper-faithful CommNet (Sukhbaatar et al. NeurIPS 2016).

    K rounds of feedforward communication per timestep -- NO GRU, no cross-
    timestep hidden state.  Each round:
        c^k_j  = mean_{j'!=j}(h^k_{j'})          (receive from peers)
        h^{k+1} = tanh(H^k @ h^k + C^k @ c^k)   (update with comm)
    Separate H^k, C^k weights per round (more expressive than tied weights).
    Compatible with shuffled PPO minibatches: every sample is self-contained.
    """
    hid_dim:       int = 256
    action_dim:    int = 8
    n_comm_rounds: int = 3
    ...
    @nn.compact
    def __call__(self, hidden, obs_img, done):
        """
        hidden  : (n_envs, n_agents, hid_dim)  -- dummy, ignored (API compat)
        obs_img : (n_envs, n_agents, H, W, C)
        done    : (n_envs,)                    -- unused (no recurrent state)
        Returns (new_hidden=zeros, logits, values).
        """
        ...
        # Encode: h^0 = tanh(W_e * CNN(obs))
        cnn_out = _CNN()(o_flat)                                    # (B*N, 64)
        h_flat = nn.tanh(
        ...
        # K rounds: c^k = mean_{j'!=j}(h^k_{j'}),  h^{k+1} = tanh(H^k h^k + C^k c^k)
        for k in range(self.n_comm_rounds):
            h = h_flat.reshape(n_envs, n_agents, self.hid_dim)
            h_sum = h.sum(axis=1, keepdims=True)                    # (B, 1, hid_dim)
            comm = (h_sum - h) / (n_agents - 1) if n_agents > 1 else jnp.zeros_like(h)
        ...
        return (
            jnp.zeros_like(hidden),                                 # dummy new_hidden
...
class Transition(NamedTuple):
    ...
    hidden:   chex.Array  # (n_envs, n_agents, hid_dim) -- h_{t-1}
...
def make_train(cfg: dict):
    ...
    net = CommNetCNN(hid_dim=hid_dim, action_dim=n_actions)
    ...
    def train(rng):
        ...
        runner_state = RunnerState(ts, hidden, env_state, obs, 0, _rng)

        def _update_step(runner_state, _unused):

            def _env_step(runner_state, _unused):
                ts, hidden, env_state, obs, update_step, key = runner_state
                key, k_act, k_step = jax.random.split(key, 3)

                new_hidden, logits, values = ts.apply_fn(
                    ts.params, hidden, obs, jnp.zeros(n_envs))
                ...
                transition = Transition(
                    ...
                    reward=reward, done=done_all, info=info,
                )
                return RunnerState(ts, new_hidden, next_env_state, next_obs,
                                   update_step, key), transition

            runner_state, traj = jax.lax.scan(_env_step, runner_state, None, n_steps)
            ts, hidden, env_state, obs, update_step, key = runner_state

            _, _, last_val = ts.apply_fn(ts.params, hidden, obs, jnp.zeros(n_envs))
            ...
            runner_state = RunnerState(ts, hidden, env_state, obs, update_step, key)
...
def save_checkpoint(params, path: Path):
    ...
    np.savez(str(path), **{f"commnet_{i}": np.array(v) for i, v in enumerate(leaves)})
...
def parse_args():
    _default_out = str(
        ...
        / "var" / "trainer" / "socialjax" / "coop_mining" / "CommNet_CNN_6agent"
    )
...
def main():
    ...
    print(f"[CommNet-CNN] coop_mining  n_agents={args.num_agents}  "
    ...
    print(f"  CommNet (Sukhbaatar et al. 2016)  K=3 feedforward comm rounds  no GRU")
    print(f"  CNN->Dense({args.hidden_dim})->[H+C comm x3]->logits")
