...
Usage (smoke):
  ...
  python -m jaxmarl_worker.algorithms.mappo_scan_socialjax \\
    --run-dir var/trainer/coop_mining/MAPPO --total-updates 50 --n-envs 7

Usage (full):
  XLA_PYTHON_CLIENT_PREALLOCATE=false \\
  python -m jaxmarl_worker.algorithms.mappo_scan_socialjax \\
    --run-dir var/trainer/coop_mining/MAPPO --total-updates 7143 --n-envs 7
"""
import os
...
from jaxmarl_worker.algorithms.mappo_indagobs_scan import make_train
...
def main():
    ...
    assert args.max_steps == 1000, f"CoopMining episode length must be exactly 1000 steps, got {args.max_steps}"
