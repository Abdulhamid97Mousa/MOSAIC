"""CommNet-CNN for SocialJax coin_game.
...
CommNet (Sukhbaatar et al., NeurIPS 2016) with SocialJax paper CNN backbone:
  ...
  C-layer on h_{t-1} (zero-init), comm_i = mean_{j!=i} C(h_j^{t-1}), into GRU
  PPO actor-critic, shared params (IPPO-CR)
  2 agents, shared rewards, 100M env steps

Run:
  CUDA_VISIBLE_DEVICES=1 XLA_PYTHON_CLIENT_PREALLOCATE=false \\
    python -m jaxmarl_worker.algorithms.socialjax.coins.commnet_cnn_scan_socialjax
"""
        ...
        def _update_step(runner_state, _unused):
            ...
            def _env_step(runner_state, _unused):
                ...
                transition = Transition(
                    obs=obs, hidden=hidden, action=acts, log_prob=logp,
                    value=values, reward=reward, done=done_all, info=info,
                )
            ...
            def _log(args):
                ...
                print(f"  update {int(step):4d}/{n_updates}  "
                      f"return={float(ret):.3f}  loss={float(loss):.4f}", flush=True)
...
def parse_args():
    _default_out = str(
        ...
        / "var" / "trainer" / "socialjax" / "coins" / "CommNet_CNN"
    )
...
def main():
    ...
    print(f"[CommNet-CNN] coin_game  n_agents={args.num_agents}  "
