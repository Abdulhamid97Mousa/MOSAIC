"""IPPO-CNN for SocialJax coin_game -- paper-faithful replication.

Exactly matches the SocialJax paper (Guo et al., ICLR 2026) config:
  - CNN backbone: Conv(32,5x5) -> Conv(32,3x3) -> Conv(32,3x3) -> Dense(64)
  - 2 agents, shared parameters, common reward (shared_rewards=True)
  - 100M env steps, 256 parallel envs, 1000 steps/episode

Run:
  CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_PREALLOCATE=false \\
    python -m jaxmarl_worker.algorithms.socialjax.coins.ippo_cnn_scan_socialjax
"""
import argparse
import sys
import time
...
def make_train(cfg: dict):
    ...
    n_actions = env_raw.action_space().n              # 7
    ...
    def train(rng):
        ...
        def _update_step(runner_state, _unused):
            def _env_step(runner_state, _unused):
                ...
                # reward: (NUM_ENVS, n_agents)  done: {'__all__': (NUM_ENVS,), '0':..., '1':...}
...
def parse_args():
    _default_out = str(
        ...
        / "var" / "trainer" / "socialjax" / "coins" / "IPPO_CNN"
    )
    ...
    p.add_argument("--num-agents",,      type=int,   default=2,    help="Paper default: 2")
...
def main():
    ...
    print(f"[IPPO-CNN] coin_game  n_agents={args.num_agents}  "
