"""MAT-CNN on SocialJax clean_up -- CNN encoder + transformer (paper-faithful).

Same Multi-Agent Transformer as coop_mining MAT but for clean_up (7 agents,
obs_c=19 channels). CNN encoder replaces flat linear projection:

  flat Linear(2299->128)   ==>   Conv(32,5x5)->Conv(32,3x3)->Conv(32,3x3)
                                  ->flatten->Dense(128)
...
Run (GPU 0):
  CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_PREALLOCATE=false \\
    python -m jaxmarl_worker.algorithms.socialjax.cleanup.mat_cnn_scan_socialjax
"""
import os
...
def _make_env(num_agents: int, num_inner_steps: int) -> SocialJaxCleanup:
    return SocialJaxCleanup(
...
def parse_args():
    _default_run_dir = str(
        ...
        / "var" / "trainer" / "socialjax" / "cleanup" / "MAT_CNN"
    )
...
def main():
    ...
    print(f"[MAT-CNN-SocialJax] clean_up  n_agents={env.num_agents}  "
