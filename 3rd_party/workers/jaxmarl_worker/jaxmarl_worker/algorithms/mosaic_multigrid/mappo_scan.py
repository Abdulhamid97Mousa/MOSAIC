"""GPU-accelerated MAPPO with a single centralised critic (global state = all agents' obs).

This is the standard cooperative MAPPO (Yu et al. 2021).  For competitive settings
where each team needs its own critic, use mappo_critic_per_team.py instead.
"""
...
def make_train(config: dict):
    ...
    obs_dim_net   = obs_dim
    gs_dim        = n_agents * obs_dim           # critic: raw obs concatenated (no agent ID needed)
    ...
    def _update_step(runner_state: RunnerState, _):
        ...
        def _env_step(runner_state: RunnerState, _):
            ...
            transition  = Transition(
                obs=obs_arr, action=acts_2d, log_prob=logp.reshape(n_envs, n_agents),
        ...
        obs_arr  = jnp.stack([obs[f"agent_{i}"] for i in range(n_agents)], axis=1)
        gs       = obs_arr.reshape(n_envs, gs_dim)
        lv_e     = jnp.asarray(critic.apply(c_st.params, gs))  # critic uses raw obs
        ...
        # Flatten: each (step, env, agent) becomes one training sample;
        # global state is tiled once per agent from the shared env state.
        flat_obs  = traj.obs.reshape(total_rollout, obs_dim_net)
        flat_gs   = jnp.repeat(
        ...
        flat_ret  = targets.reshape(total_rollout)
        flat_val  = traj.value.reshape(total_rollout)  # old values for clipped VF loss
        ...
        def _epoch(carry, _):
            ...
            s_ret  = flat_ret[perm];  s_gs   = flat_gs[perm]
            s_val  = flat_val[perm]
            ...
            def _mb(carry, batch):
                a_st, c_st = carry
                obs_b, act_b, logp_b, adv_b, ret_b, gs_b, val_b = batch
                ...
                def critic_loss_fn(params):
                    vals      = jnp.asarray(critic.apply(params, gs_b))
                    val_clip  = val_b + jnp.clip(vals - val_b, -clip_eps, clip_eps)
                    vf_loss   = 0.5 * jnp.maximum(
                        jnp.square(vals - ret_b),
                        jnp.square(val_clip - ret_b),
                    ).mean()
                    return vf_coef * vf_loss, vf_loss

                (al, (pgl, ent)), ag = jax.value_and_grad(actor_loss_fn,  has_aux=True)(a_st.params)
                (cl, vfl),        cg = jax.value_and_grad(critic_loss_fn, has_aux=True)(c_st.params)
                a_st = a_st.apply_gradients(grads=ag)
                c_st = c_st.apply_gradients(grads=cg)
                return (a_st, c_st), {"actor_loss": al, "critic_loss": cl,
                                      "entropy": ent, "pg_loss": pgl, "vf_loss": vfl}

            (a_st, c_st), mb_m = jax.lax.scan(
                _mb, (a_st, c_st),
                (mb_obs, mb_act, mb_logp, mb_adv, mb_ret, mb_gs, mb_val),
            )
            return (a_st, c_st, key), mb_m
...
def main(cfg: TrainConfig) -> None:
    ...
    print(f"[MAPPO-scan] {cfg.env.family}-{cfg.env.variant}  "
          f"n_agents={env.num_agents}  n_envs={cfg.ppo.n_envs}  "
          f"n_steps={cfg.ppo.n_steps}  total_updates={cfg.total_updates}  "
          f"obs_dim={env._obs_dim}  gs_dim={env.num_agents * env._obs_dim}  mode=global-critic")
    ...
    print("[MAPPO-scan] JIT-compiling training function...", flush=True)
    t0  = time.time()
    ...
    for chunk_idx in range(n_chunks):
        ...
        print(f"[MAPPO-scan] chunk {chunk_idx + 1}/{n_chunks}  "
    ...
    print(f"[MAPPO-scan] Training complete in {t1 - t0:.1f}s  ({sps:,.0f} steps/sec)")

    ep_rets = np.concatenate(ep_rets_chunks)
    print(f"  final ep_ret (last 50 updates): {ep_rets[-50:].mean():.4f}")
    print(f"  actor_loss:  {float(al_chunks[-1][-1]):.4f}")
    print(f"  critic_loss: {float(cl_chunks[-1][-1]):.4f}")
    ...
    c_leaves, _ = jax.tree_util.tree_flatten(
        jax.tree_util.tree_map(np.array, runner_state.critic_state.params))
    final_path = ckpt_dir / "final.npz"
    np.savez(str(final_path), *a_leaves, *c_leaves)
    print(f"[MAPPO-scan] checkpoint → {final_path}")
