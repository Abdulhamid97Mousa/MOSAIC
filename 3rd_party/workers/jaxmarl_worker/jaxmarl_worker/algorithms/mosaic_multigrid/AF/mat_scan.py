...
def main():
    ...
    assert "af" in ["af", "bb", "soccer"], "env_family must be 'af', 'bb', or 'soccer', got af"
    ...
    ckpt_dir = run_dir / "checkpoints" / args.training_reward / f"af-{args.variant}"
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    env = _make_env("af", args.variant, view_size=args.view_size,
                    ball_coef=args.ball_approach_coef, max_steps=args.max_steps,
                    goal_rows=args.goal_rows, cooperative=args.cooperative)
    print(f"[MAT-scan] af-{args.variant}  n_agents={env.num_agents}  "
