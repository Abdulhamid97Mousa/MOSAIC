...
def main():
    ...
    assert "bb" in ["af", "bb", "soccer"], "env_family must be 'af', 'bb', or 'soccer', got bb"
    ...
    ckpt_dir = run_dir / "checkpoints" / args.training_reward / f"bb-{args.variant}"
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    env = _make_env("bb", args.variant, view_size=args.view_size,
                    ball_coef=args.ball_approach_coef, max_steps=args.max_steps,
                    goal_rows=args.goal_rows, cooperative=args.cooperative)
    print(f"[MAT-scan] bb-{args.variant}  n_agents={env.num_agents}  "
    ...
    if args.tensorboard:
        ...
        def _tb_callback(step, ep_ret, loss):
            tb_writer.add_scalar("train/episode_return", float(ep_ret), int(step))
            tb_writer.add_scalar("train/total_loss", float(loss), int(step))
            tb_writer.flush()
