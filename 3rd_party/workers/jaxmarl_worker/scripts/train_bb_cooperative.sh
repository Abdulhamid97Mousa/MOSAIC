#!/bin/bash
# BB cooperative training — IPPO, MAPPO, HAPPO, QMIX, VDPPO
# Variants: G-2v0 (2 agents vs 0) and G-3v0 (3 agents vs 0)
# Output: var/trainer/mosaic_multigrid/BB/cooperative/{2v0,3v0}/{ALG}/
#
# IPPO/MAPPO/HAPPO/VDPPO are Hydra-driven (Phase 1 refactor, 2026-09-18):
# sport is env=bb (not a BB.<alg>_scan module path), flags are key=value.
# QMIX remains the pre-refactor argparse script (Phase 2, not yet migrated).
#
# Usage:
#   GPU=0 bash scripts/train_bb_cooperative.sh
#   GPU=0 VARIANTS="G-2v0" bash scripts/train_bb_cooperative.sh   # single variant
#   GPU=0 SKIP_QMIX=1 bash scripts/train_bb_cooperative.sh        # skip QMIX
...
RUNS_ROOT="$PROJECT_ROOT/var/trainer/mosaic_multigrid/BB/cooperative"
VARIANTS="${VARIANTS:-G-2v0 G-3v0}"

# Shared PPO hyperparams (Hydra key=value overrides; env=bb supplies
# view_size/ball_approach_coef/goal_rows/max_steps defaults from conf/env/bb.yaml)
PPO_ARGS=(
    env=bb
...
run_alg() {
    ...
    shift 3
    local VSHORT="${VARIANT##*-}"          # G-2v0 → 2v0
    local RUN_DIR="$RUNS_ROOT/$VSHORT/$ALG"
    local CKPT="$RUN_DIR/checkpoints/bb-$VARIANT/final.npz"
    if [ -f "$CKPT" ]; then
...
echo "========================================================"
echo "  BB cooperative training  GPU=$GPU"
...
for VARIANT in $VARIANTS; do
    ...
    [ -z "${SKIP_QMIX:-}" ]  && run_alg_qmix "$VARIANT" QMIX jaxmarl_worker.algorithms.mosaic_multigrid.BB.qmix_scan "${QMIX_ARGS[@]}"
done
