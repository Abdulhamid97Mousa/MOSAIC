#!/bin/bash
# AF cooperative training — IPPO, MAPPO, HAPPO, QMIX, VDPPO
# Variants: G-2v0 and G-3v0
# Output: var/trainer/mosaic_multigrid/AF/cooperative/{2v0,3v0}/{ALG}/
#
# IPPO/MAPPO/HAPPO/VDPPO are Hydra-driven (Phase 1 refactor, 2026-09-18):
# sport is env=af (not an AF.<alg>_scan module path), flags are key=value.
# QMIX remains the pre-refactor argparse script (Phase 2, not yet migrated).
#
# Usage:
#   GPU=0 bash scripts/train_af_cooperative.sh
#   GPU=0 VARIANTS="G-2v0" bash scripts/train_af_cooperative.sh
...
RUNS_ROOT="$PROJECT_ROOT/var/trainer/mosaic_multigrid/AF/cooperative"
VARIANTS="${VARIANTS:-G-2v0 G-3v0}"

# Shared PPO hyperparams (Hydra key=value overrides; env=af supplies
# view_size/ball_approach_coef/goal_rows/max_steps defaults from conf/env/af.yaml)
PPO_ARGS=(
    env=af
...
run_alg() {
    ...
    local CKPT="$RUN_DIR/checkpoints/af-$VARIANT/final.npz"
    if [ -f "$CKPT" ]; then
...
echo "========================================================"
echo "  AF cooperative training  GPU=$GPU"
...
for VARIANT in $VARIANTS; do
    ...
    [ -z "${SKIP_QMIX:-}" ]  && run_alg_qmix "$VARIANT" QMIX jaxmarl_worker.algorithms.mosaic_multigrid.AF.qmix_scan "${QMIX_ARGS[@]}"
done
