"""FastLane telemetry helpers for the TorchRL worker.

Thin module that reuses CleanRL's FastLaneTelemetryWrapper to provide
real-time frame streaming for TorchRL training runs.

Note: TorchRL uses torchrl.envs.GymEnv which internally calls gym.make,
so patching gym.make via sitecustomize.py is sufficient.
"""
...
def _resolve_config() -> _FastLaneConfig:
    ...
    try:
        total_envs = max(1, int(os.getenv("TORCHRL_NUM_ENVS", os.getenv("NUM_ENVS", "1"))))
    except ValueError:
        total_envs = 1

    slot = max(0, min(slot, total_envs - 1))

    run_id = os.getenv("TORCHRL_RUN_ID") or os.getenv("RUN_ID") or "torchrl-run"
    agent_id = os.getenv("TORCHRL_AGENT_ID") or os.getenv("AGENT_ID") or "torchrl-agent"
    worker_id = (
        os.getenv("WORKER_ID")
        or os.getenv("TORCHRL_WORKER_ID")
        or "torchrl-worker"
    )

    try:
        seed = int(os.getenv("TORCHRL_SEED", os.getenv("SEED", "0")))
    except ValueError:
        seed = 0
...
def maybe_wrap_env(env: Any) -> Any:
    ...
    try:
        ...
        _LOGGER.debug("[TORCHRL-FASTLANE] Failed to wrap env at slot %d", slot_index)
        return env
