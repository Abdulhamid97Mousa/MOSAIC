"""Configuration parsing helpers for the TorchRL worker shim."""
...
@dataclass
class TorchRLWorkerConfig:
    """Structured configuration for launching a TorchRL worker run.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("torchrl_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("torchrl_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("torchrl_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("torchrl_worker config missing required field 'total_timesteps'")
        ...
        if WorkerConfigProtocol is not None:
            assert isinstance(self, WorkerConfigProtocol), (
                "TorchRLWorkerConfig must implement WorkerConfig protocol"
            )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "TorchRLWorkerConfig":
...
def load_worker_config(path: Path) -> TorchRLWorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return TorchRLWorkerConfig.from_dict(worker_config)

    return TorchRLWorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = TorchRLWorkerConfig
