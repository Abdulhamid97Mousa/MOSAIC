...
Location: 3rd_party/environments/multigrid_sports/
...
class GymToGymnasiumWrapper(_gymnasium.Env if _HAS_GYMNASIUM else object):
    ...
    multigrid_sports v4.4.0 uses Gymnasium API natively, but this wrapper
...
def _get_env_class(env_id: str) -> Optional[Type]:
    ...
    This prevents import errors if multigrid_sports is not installed.
    """
    ...
    # The wrapper historically imported ``multigrid_sports.envs`` throughout.
    # v7 renamed that distribution/module to ``mosaic_multigrid``.  Install a
    # narrow in-process compatibility alias so the existing short-name-to-class
    # table keeps working without reviving removed Gymnasium IDs.
    try:
        import mosaic_multigrid as _mosaic_multigrid
        import mosaic_multigrid.envs as _mosaic_envs

        sys.modules.setdefault("multigrid_sports", _mosaic_multigrid)
        sys.modules["multigrid_sports.envs"] = _mosaic_envs

        if env_id.lower() in ("soccer", "soccergame4hindagobsenv16x11n2"):
            from multigrid_sports.envs import SoccerGame4HIndAgObsEnv16x11N2
            env_cls = SoccerGame4HIndAgObsEnv16x11N2

        elif env_id.lower() in ("soccer_1vs1", "soccergame2hindagobsenv16x11n2"):
            from multigrid_sports.envs import SoccerGame2HIndAgObsEnv16x11N2
            env_cls = SoccerGame2HIndAgObsEnv16x11N2

        elif env_id.lower() in ("collect", "collectgame4hindagobsenv10x10n2"):
            from multigrid_sports.envs import CollectGame4HIndAgObsEnv10x10N2
            env_cls = CollectGame4HIndAgObsEnv10x10N2

        elif env_id.lower() in ("collect_1vs1", "collectgame2hindagobsenv10x10n2"):
            from multigrid_sports.envs import CollectGame2HIndAgObsEnv10x10N2
        ...
        # --- 3vs3 IndAgObs variants ---
            ...
            from multigrid_sports.envs import BasketballGame6HIndAgObsEnv19x11N3
        ...
        # --- TeamObs variants (IndAgObs + teammate features) ---
            ...
            from multigrid_sports.envs import SoccerTeamObsEnv
            env_cls = SoccerTeamObsEnv

        elif env_id.lower() in ("collect_2vs2_teamobs", "collect2v2teamobsenv"):
            from multigrid_sports.envs import Collect2v2TeamObsEnv
            env_cls = Collect2v2TeamObsEnv

        elif env_id.lower() in ("basketball_3vs3_teamobs", "basketball3v3teamobsenv"):
            from multigrid_sports.envs import Basketball3v3TeamObsEnv
            env_cls = Basketball3v3TeamObsEnv

        # --- Solo variants (v6.0.0, single-agent, no opponent) ---
        elif env_id.lower() in ("soccer_solo_green", "soccersologreenindagobsenv16x11",
                                "soccer_g_1v0"):
            from multigrid_sports.envs import SoccerSoloGreenIndAgObsEnv16x11
            env_cls = SoccerSoloGreenIndAgObsEnv16x11

        elif env_id.lower() in ("soccer_solo_blue", "soccersoloblueindagobsenv16x11",
                                "soccer_b_0v1"):
            from multigrid_sports.envs import SoccerSoloBlueIndAgObsEnv16x11
            env_cls = SoccerSoloBlueIndAgObsEnv16x11

        elif env_id.lower() in ("basketball_solo_green", "basketballsologreenindagobsenv19x11",
                                "basketball_g_1v0"):
            from multigrid_sports.envs import BasketballSoloGreenIndAgObsEnv19x11
            env_cls = BasketballSoloGreenIndAgObsEnv19x11

        elif env_id.lower() in ("basketball_solo_blue", "basketballsoloblueindagobsenv19x11",
                                "basketball_b_0v1"):
            from multigrid_sports.envs import BasketballSoloBlueIndAgObsEnv19x11
            env_cls = BasketballSoloBlueIndAgObsEnv19x11

        # --- American Football variants (v7.0.0) ---
        elif env_id.lower() in ("american_football_1v1", "americanfootball1v1env16x11",
                                "af_1v1_indagobs"):
            from multigrid_sports.envs import AmericanFootball1v1Env16x11
            env_cls = AmericanFootball1v1Env16x11

        elif env_id.lower() in ("american_football_2v2", "americanfootball2v2env16x11",
                                "af_2v2_indagobs"):
            from multigrid_sports.envs import AmericanFootball2v2Env16x11
            env_cls = AmericanFootball2v2Env16x11

        elif env_id.lower() in ("american_football_solo_green", "americanfootballsologreenenv16x11",
                                "af_g_1v0"):
            from multigrid_sports.envs import AmericanFootballSoloGreenEnv16x11
            env_cls = AmericanFootballSoloGreenEnv16x11

        elif env_id.lower() in ("american_football_solo_blue", "americanfootballsoloblueenv16x11",
                                "af_b_0v1"):
            from multigrid_sports.envs import AmericanFootballSoloBlueEnv16x11
            env_cls = AmericanFootballSoloBlueEnv16x11

        # ── v1 environment aliases (S/BB/AF abbreviations, view_size=7) ─────
        # Soccer solo cooperative (2v0, 3v0, 0v2, 0v3)
        elif env_id.lower() == "soccer_g_2v0_indagobs":
            from multigrid_sports.envs import SoccerGreen2v0IndAgObsEnv16x11
            env_cls = SoccerGreen2v0IndAgObsEnv16x11
        elif env_id.lower() == "soccer_g_2v0_teamobs":
            from multigrid_sports.envs import SoccerGreen2v0TeamObsEnv
            env_cls = SoccerGreen2v0TeamObsEnv
        elif env_id.lower() == "soccer_b_0v2_indagobs":
            from multigrid_sports.envs import SoccerBlue0v2IndAgObsEnv16x11
            env_cls = SoccerBlue0v2IndAgObsEnv16x11
        elif env_id.lower() == "soccer_b_0v2_teamobs":
            from multigrid_sports.envs import SoccerBlue0v2TeamObsEnv
            env_cls = SoccerBlue0v2TeamObsEnv
        elif env_id.lower() == "soccer_g_3v0_indagobs":
            from multigrid_sports.envs import SoccerGreen3v0IndAgObsEnv16x11
            env_cls = SoccerGreen3v0IndAgObsEnv16x11
        elif env_id.lower() == "soccer_g_3v0_teamobs":
            from multigrid_sports.envs import SoccerGreen3v0TeamObsEnv
            env_cls = SoccerGreen3v0TeamObsEnv
        elif env_id.lower() == "soccer_b_0v3_indagobs":
            from multigrid_sports.envs import SoccerBlue0v3IndAgObsEnv16x11
            env_cls = SoccerBlue0v3IndAgObsEnv16x11
        elif env_id.lower() == "soccer_b_0v3_teamobs":
            from multigrid_sports.envs import SoccerBlue0v3TeamObsEnv
            env_cls = SoccerBlue0v3TeamObsEnv
        # Soccer adversarial v1 aliases
        elif env_id.lower() in ("soccer_1v1_indagobs",):
            from multigrid_sports.envs import SoccerGame2HIndAgObsEnv16x11N2
            env_cls = SoccerGame2HIndAgObsEnv16x11N2
        elif env_id.lower() in ("soccer_2v2_indagobs",):
            from multigrid_sports.envs import SoccerGame4HIndAgObsEnv16x11N2
            env_cls = SoccerGame4HIndAgObsEnv16x11N2
        elif env_id.lower() in ("soccer_2v2_teamobs",):
            from multigrid_sports.envs import SoccerTeamObsEnv
            env_cls = SoccerTeamObsEnv
        elif env_id.lower() == "soccer_3v3_indagobs":
            from multigrid_sports.envs import SoccerGame6HIndAgObsEnv16x11N3
            env_cls = SoccerGame6HIndAgObsEnv16x11N3
        # Basketball v1
        elif env_id.lower() == "basketball_g_2v0_indagobs":
            from multigrid_sports.envs import BasketballGreen2v0IndAgObsEnv19x11
            env_cls = BasketballGreen2v0IndAgObsEnv19x11
        elif env_id.lower() == "basketball_g_2v0_teamobs":
            from multigrid_sports.envs import BasketballGreen2v0TeamObsEnv
            env_cls = BasketballGreen2v0TeamObsEnv
        elif env_id.lower() == "basketball_b_0v2_indagobs":
            from multigrid_sports.envs import BasketballBlue0v2IndAgObsEnv19x11
            env_cls = BasketballBlue0v2IndAgObsEnv19x11
        elif env_id.lower() == "basketball_b_0v2_teamobs":
            from multigrid_sports.envs import BasketballBlue0v2TeamObsEnv
            env_cls = BasketballBlue0v2TeamObsEnv
        elif env_id.lower() == "basketball_g_3v0_indagobs":
            from multigrid_sports.envs import BasketballGreen3v0IndAgObsEnv19x11
            env_cls = BasketballGreen3v0IndAgObsEnv19x11
        elif env_id.lower() == "basketball_g_3v0_teamobs":
            from multigrid_sports.envs import BasketballGreen3v0TeamObsEnv
            env_cls = BasketballGreen3v0TeamObsEnv
        elif env_id.lower() == "basketball_b_0v3_indagobs":
            from multigrid_sports.envs import BasketballBlue0v3IndAgObsEnv19x11
            env_cls = BasketballBlue0v3IndAgObsEnv19x11
        elif env_id.lower() == "basketball_b_0v3_teamobs":
            from multigrid_sports.envs import BasketballBlue0v3TeamObsEnv
            env_cls = BasketballBlue0v3TeamObsEnv
        elif env_id.lower() == "basketball_1v1_indagobs":
            from multigrid_sports.envs import BasketballGame2HIndAgObsEnv19x11N1
            env_cls = BasketballGame2HIndAgObsEnv19x11N1
        elif env_id.lower() == "basketball_2v2_indagobs":
            from multigrid_sports.envs import BasketballGame4HIndAgObsEnv19x11N2
            env_cls = BasketballGame4HIndAgObsEnv19x11N2
        elif env_id.lower() in ("basketball_3v3_indagobs",):
            from multigrid_sports.envs import BasketballGame6HIndAgObsEnv19x11N3
            env_cls = BasketballGame6HIndAgObsEnv19x11N3
        # American Football v1
        elif env_id.lower() == "af_g_2v0_indagobs":
            from multigrid_sports.envs import AmericanFootballGreen2v0Env16x11
            env_cls = AmericanFootballGreen2v0Env16x11
        elif env_id.lower() == "af_g_2v0_teamobs":
            from multigrid_sports.envs import AmericanFootballGreen2v0TeamObsEnv16x11
            env_cls = AmericanFootballGreen2v0TeamObsEnv16x11
        elif env_id.lower() == "af_b_0v2_indagobs":
            from multigrid_sports.envs import AmericanFootballBlue0v2Env16x11
            env_cls = AmericanFootballBlue0v2Env16x11
        elif env_id.lower() == "af_b_0v2_teamobs":
            from multigrid_sports.envs import AmericanFootballBlue0v2TeamObsEnv16x11
            env_cls = AmericanFootballBlue0v2TeamObsEnv16x11
        elif env_id.lower() == "af_g_3v0_indagobs":
            from multigrid_sports.envs import AmericanFootballGreen3v0Env16x11
            env_cls = AmericanFootballGreen3v0Env16x11
        elif env_id.lower() == "af_g_3v0_teamobs":
            from multigrid_sports.envs import AmericanFootballGreen3v0TeamObsEnv16x11
            env_cls = AmericanFootballGreen3v0TeamObsEnv16x11
        elif env_id.lower() == "af_b_0v3_indagobs":
            from multigrid_sports.envs import AmericanFootballBlue0v3Env16x11
            env_cls = AmericanFootballBlue0v3Env16x11
        elif env_id.lower() == "af_b_0v3_teamobs":
            from multigrid_sports.envs import AmericanFootballBlue0v3TeamObsEnv16x11
            env_cls = AmericanFootballBlue0v3TeamObsEnv16x11
        elif env_id.lower() == "af_2v2_teamobs":
            from multigrid_sports.envs import AmericanFootball2v2TeamObsEnv16x11
            env_cls = AmericanFootball2v2TeamObsEnv16x11
        elif env_id.lower() == "af_3v3_indagobs":
            from multigrid_sports.envs import AmericanFootball3v3Env16x11
            env_cls = AmericanFootball3v3Env16x11
        elif env_id.lower() == "af_3v3_teamobs":
            from multigrid_sports.envs import AmericanFootball3v3TeamObsEnv16x11
            env_cls = AmericanFootball3v3TeamObsEnv16x11
...
def get_available_environments() -> List[str]:
    """Return v7 short IDs backed by classes in ``mosaic_multigrid``."""
    available = []
    for env_id in [
        "soccer_g_1v0", "soccer_b_0v1",
        "soccer_1v1_indagobs", "soccer_2v2_indagobs", "soccer_3v3_indagobs",
        "soccer_g_2v0_indagobs", "soccer_g_3v0_indagobs",
        "soccer_b_0v2_indagobs", "soccer_b_0v3_indagobs",
        "basketball_g_1v0", "basketball_b_0v1",
        "basketball_1v1_indagobs", "basketball_2v2_indagobs",
        "basketball_3v3_indagobs",
        "basketball_g_2v0_indagobs", "basketball_g_3v0_indagobs",
        "basketball_b_0v2_indagobs", "basketball_b_0v3_indagobs",
        "af_g_1v0", "af_b_0v1",
        "af_1v1_indagobs", "af_2v2_indagobs", "af_3v3_indagobs",
        "af_g_2v0_indagobs", "af_g_3v0_indagobs",
        "af_b_0v2_indagobs", "af_b_0v3_indagobs",
        "collect_1vs1", "collect_2vs2_indagobs",
    ]:
...
MULTIGRID_ENV_INFO = {
    ...
    # --- American Football variants (v7.0.0) ---
    "american_football_1v1": {
        "full_name": "AmericanFootball1v1Env16x11",
        "description": "1v1 American Football - touchdown scoring, 16x11 field, first-to-2",
        "num_agents": 2,
        "default_teams": [[0], [1]],
        "team_names": ["green", "blue"],
        "recommended_mode": TrainingMode.COMPETITIVE,
    ...
    "american_football_2v2": {
        "full_name": "AmericanFootball2v2Env16x11",
        "description": "2v2 American Football - touchdown scoring, 16x11 field, first-to-2",
        "num_agents": 4,
        "default_teams": [[0, 1], [2, 3]],
        "team_names": ["green", "blue"],
        "recommended_mode": TrainingMode.COMPETITIVE,
    ...
    "american_football_solo_green": {
        "full_name": "AmericanFootballSoloGreenEnv16x11",
        "description": "Solo American Football (Green) - score touchdown, no opponent",
        "num_agents": 1,
        "default_teams": [[0]],
        "team_names": ["green"],
        "recommended_mode": TrainingMode.INDEPENDENT,
    ...
    "american_football_solo_blue": {
        "full_name": "AmericanFootballSoloBlueEnv16x11",
        "description": "Solo American Football (Blue) - score touchdown, no opponent",
        "num_agents": 1,
        "default_teams": [[0]],
        "team_names": ["blue"],
        "recommended_mode": TrainingMode.INDEPENDENT,
...
class SoloMultiGrid_Env(_gymnasium.Env if _HAS_GYMNASIUM else object):
    """Standard Gymnasium wrapper for solo multigrid_sports environments.
    ...
    Action: Discrete(8) — noop, left, right, forward, pickup, drop, toggle, done
    ...
    def __init__(self, config: Any) -> None:
        ...
        # Pass view_size from MOSAIC_VIEW_SIZE env var (same as MultiGrid_Env)
        env_kwargs = {"render_mode": render_mode}
        view_size_str = os.environ.get("MOSAIC_VIEW_SIZE", "")
        if view_size_str:
            env_kwargs["view_size"] = int(view_size_str)
            _logger.info(f"Solo env using view_size={view_size_str} from MOSAIC_VIEW_SIZE")
        self._inner = env_cls(**env_kwargs)
    ...
    def step(self, action):
        # MultiGridEnv.handle_actions expects a dict keyed by agent index.
        # Passing a list silently drops non-zero actions because
        # `i in [a]` tests value-membership, not index-membership.
        result = self._inner.step({0: int(action)})
...
class MultiGrid_Env(RawMultiAgentEnv):
    """XuanCe-compatible wrapper for multigrid_sports environments.

    This wrapper converts multigrid_sports's list-based multi-agent API to
    XuanCe's dict-based API, with flexible training mode support.

    Attributes:
        env: The wrapped multigrid_sports environment
    ...
    def __init__(self, config: Any) -> None:
        ...
        if env_cls is None:
            ...
            raise ValueError(
                ...
                f"Make sure multigrid_sports is installed."
            )
        ...
        # multigrid_sports uses Gymnasium API - no wrapper needed (already reproducible)
        ...
        self._base_env_agents = _base.agents  # multigrid_sports agent objects
        ...
        # multigrid_sports uses Gymnasium API (5-tuple step return) - no conversion needed
        ...
        # multigrid_sports uses dict observation_space: {0: Dict[image, direction, mission], 1: ...}
        ...
        # multigrid_sports uses dict action_space: {0: Discrete(8), 1: Discrete(8), ...}
    ...
    def _build_teams_from_env(self) -> List[List[str]]:
        """Build team groups from multigrid_sports's agent.index assignments.
    ...
    def reset(self, **kwargs: Any) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        # multigrid_sports returns dict {0: {image, direction, mission, ...}, 1: ...}
    ...
    def step(
        ...
        """
        # multigrid_sports uses dict actions: {0: action, 1: action, ...}
        actions_dict = {i: actions[f"agent_{i}"] for i in range(self.num_agents)}

        # multigrid_sports uses Gymnasium API (5-tuple dict-keyed return)
    ...
    def avail_actions(self) -> Dict[str, np.ndarray]:
        ...
        When MOSAIC_MASK_INVALID_ACTIONS=1 is set, masks actions that are
        irrelevant for Soccer/Basketball (noop=0, toggle=6, done=7), reducing
        the effective action space from 8 to 5 actions while keeping the same
        policy output dimensions (clean deployment, no wrappers needed).

        Set MOSAIC_INVALID_ACTIONS=0,6,7 to customise which actions to mask.
        """
        num_actions = self.action_space[self.agents[0]].n
        if os.environ.get("MOSAIC_MASK_INVALID_ACTIONS", "0") == "1":
            invalid = [
                int(a)
                for a in os.environ.get("MOSAIC_INVALID_ACTIONS", "0,6,7").split(",")
            ]
            mask = np.ones(num_actions, dtype=np.bool_)
            for idx in invalid:
                if 0 <= idx < num_actions:
                    mask[idx] = False
            return {agent: mask.copy() for agent in self.agents}
        return {
