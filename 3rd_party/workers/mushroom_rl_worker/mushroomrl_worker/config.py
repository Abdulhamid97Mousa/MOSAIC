"""Configuration parsing helpers for the MushroomRL worker shim."""
...
@dataclass
class MushroomRLWorkerConfig:
    """Structured configuration for launching a MushroomRL worker run.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("mushroomrl_worker config missing required field 'run_id'")
        if not self.algo:
            raise ValueError("mushroomrl_worker config missing required field 'algo'")
        if not self.env_id:
            raise ValueError("mushroomrl_worker config missing required field 'env_id'")
        if self.total_timesteps is None:
            raise ValueError("mushroomrl_worker config missing required field 'total_timesteps'")
        ...
        if WorkerConfigProtocol is not None:
            assert isinstance(self, WorkerConfigProtocol), (
                "MushroomRLWorkerConfig must implement WorkerConfig protocol"
            )
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "MushroomRLWorkerConfig":
        ...
        Returns:
            MushroomRLWorkerConfig instance
        """
...
def load_worker_config(path: Path) -> MushroomRLWorkerConfig:
    ...
    if isinstance(payload, Mapping) and "metadata" in payload:
        ...
        return MushroomRLWorkerConfig.from_dict(worker_config)

    return MushroomRLWorkerConfig.from_dict(payload)


# Type alias for backwards compatibility
WorkerConfig = MushroomRLWorkerConfig
