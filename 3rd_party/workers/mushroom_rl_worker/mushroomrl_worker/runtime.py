"""Runtime orchestration helpers for launching MushroomRL algorithms."""
...
class MushroomRLWorkerRuntime:
    """Orchestrates MushroomRL training jobs."""

    def __init__(self, work_dir: Path | str) -> None:
        self.work_dir = Path(work_dir)
        self.work_dir.mkdir(parents=True, exist_ok=True)

    def execute(self, config: MushroomRLWorkerConfig) -> None:
        """Execute a MushroomRL training run based on configuration.

        Args:
            config: MushroomRLWorkerConfig instance
        """
        if _HAS_GUI:
            log_constant(
                LOGGER,
            ...
            LOGGER.info(
                "MushroomRL runtime started: run_id=%s algo=%s env_id=%s",
        ...
        try:
            ...
            if return_code != 0:
                raise RuntimeError(f"MushroomRL worker process failed with exit code {return_code}")
            ...
            if _HAS_GUI:
                ...
                LOGGER.info("MushroomRL runtime completed: run_id=%s", config.run_id)
            ...
            if _HAS_GUI:
                ...
                LOGGER.exception("MushroomRL runtime failed: run_id=%s", config.run_id)
            raise
