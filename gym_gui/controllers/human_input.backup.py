...
from typing import Callable, Dict, Iterable, List, Optional, Set, Tuple, cast, TYPE_CHECKING
...
if TYPE_CHECKING:
    from gym_gui.controllers.evdev_keyboard_monitor import EvdevKeyboardMonitor
...
from gym_gui.controllers.malmo_input import MalmoEnvKeyCombinationResolver
from gym_gui.controllers.vizdoom_input import ViZDoomKeyCombinationResolver
...
def get_key_combination_resolver(
    ...
    if family == EnvironmentFamily.MALMOENV:
        ...
        # Pass action_space so resolver can map by command string for each mission.
        return MalmoEnvKeyCombinationResolver(action_space=action_space)
...
class HumanInputController(QtCore.QObject, LogConstantMixin):
    ...
    def __init__(self, widget: QtWidgets.QWidget, session: SessionController) -> None:
        ...
        # Game-tick loop for evdev multi-keyboard human play.
        #
        # Uses singleShot chaining (the standard Qt game-loop pattern):
        # each tick does env.step(), then returns control to the Qt event
        # loop so that paint events, user input, and other housekeeping
        # can run.  Only AFTER the event loop processes pending events
        # does the next tick fire.
        #
        # This prevents event-loop starvation: a repeating QTimer would
        # fire the next tick immediately after the slot returns (coalesced),
        # never giving Qt a chance to process the paint events that make
        # the rendered frame appear on screen.
        #
        # Default target interval: 100 ms (10 Hz).
        self._evdev_tick_interval_ms = 100  # target ms between ticks
    ...
    def eventFilter(self, obj: QtCore.QObject, event: QtCore.QEvent) -> bool:
        ...
        if event_type == QtCore.QEvent.Type.KeyPress:
            ...
            if key not in self._pressed_keys:
                ...
                # Send native key press to Minecraft via TCP:9001 (MalmoEnv only).
                # This runs IN ADDITION to the step loop — the step loop still
                # needs to run to fetch observations. The native path handles the
                # actual movement; the step loop fetches frames.
                if self._session:
                    self._session.handle_native_key(key, True)
                # Emit action through step loop (fetches observations for all envs)
        ...
        if event_type == QtCore.QEvent.Type.KeyRelease:
            ...
            # Send native key release for MalmoEnv (stops movement in continuous missions).
            if self._session:
                self._session.handle_native_key(key, False)
            return True  # Consume the event
    ...
    def _get_action_name(self, action: int) -> str:
        ...
        if isinstance(self._key_resolver, INIMultiGridKeyCombinationResolver):
            # Try importing multigrid actions (safely)
            try:
                # pylint: disable=import-outside-toplevel
                from multigrid.core.actions import Action as INIAction  # type: ignore
    ...
    def setup_evdev_keyboards(self, device_assignments: Dict[str, str]) -> bool:
        ...
        # Try importing evdev modules safely
        try:
            from gym_gui.controllers.evdev_keyboard_monitor import EvdevKeyboardMonitor
            from gym_gui.controllers.keycode_translation import linux_keycode_to_qt_key
            ...
            # Start the game-tick chain (singleShot chaining).
            # Key events only update state; the chain drives stepping.
            if not self._evdev_tick_active:
                self._evdev_tick_active = True
                QTimer.singleShot(
                    self._evdev_tick_interval_ms, self._on_evdev_game_tick
                )
                _LOGGER.info(
                    f"Evdev game-tick chain started at "
                    f"{self.get_evdev_tick_rate()} Hz"
                )
    ...
    def stop_evdev_monitoring(self) -> None:
        ...
        if self._evdev_monitor:
            ...
            # Setting _evdev_tick_active to False breaks the singleShot
            # chain: the next tick will see the flag and not re-schedule.
            self._evdev_tick_active = False
    ...
    def _on_evdev_key_pressed(self, device_path: str, keycode: int, timestamp: int) -> None:
        ...
        if qt_key not in self._agent_pressed_keys[agent_id]:
            ...
            # State-only update: the game-tick timer (_on_evdev_game_tick)
            # will read _agent_pressed_keys on its next tick and step
            # the environment.  We do NOT step here.
    ...
    def _on_evdev_game_tick(self) -> None:
        """Single-shot game tick for human multi-keyboard play.

        Uses singleShot chaining: this method runs one tick, then
        schedules the next tick via ``QTimer.singleShot()``.  Between
        ticks, control returns to the Qt event loop so paint events
        can be processed and the rendered frame appears on screen.

        A repeating QTimer would starve the event loop because
        ``adapter.step()`` calls ``render()`` synchronously on the
        main thread.  By the time the slot returns, the repeating
        timer has already expired (coalesced) and fires immediately,
        never letting paint events through.

        If no agent has any keys pressed, the tick is skipped but the
        chain continues (so held keys still produce continuous movement).
        """
        if not self._evdev_tick_active:
            return

        # Only step when at least one human is pressing a key
        any_keys = any(
            bool(keys) for keys in self._agent_pressed_keys.values()
        )
        if any_keys:
            import time
            t0 = time.monotonic()
            ...
            # Subtract elapsed time from the interval so the cadence
        ...
        # Schedule the next tick.  Control returns to the event loop NOW,
        # allowing Qt to process paint events before the next tick fires.
        QTimer.singleShot(int(remaining), self._on_evdev_game_tick)

    # -- tick-rate configuration --

    def set_evdev_tick_rate(self, hz: int) -> None:
        """Set the game-tick rate for evdev human play.

        Args:
            hz: Ticks per second (e.g. 10 for 100 ms per tick).
                 Clamped to 1-60 Hz.
        """
        hz = max(1, min(60, hz))
        self._evdev_tick_interval_ms = max(1, 1000 // hz)
        _LOGGER.info(
            f"Evdev game-tick rate set to {hz} Hz "
            f"({self._evdev_tick_interval_ms} ms)"
        )

    def get_evdev_tick_rate(self) -> int:
    ...
    def _on_evdev_error(self, message: str) -> None:
