...
class MainWindow(QtWidgets.QMainWindow, LogConstantMixin):
    ...
    def __init__(self, settings: Settings, parent: QtWidgets.QWidget | None = None) -> None:
        ...
        # Keyboard worker bridge for multi-keyboard human play (subprocess-based)
        ...
        self._pettingzoo_current_seed: int | None = 42
    ...
    def _connect_signals(self) -> None:
        ...
        self._render_tabs.mouse_delta_input.connect(self._on_mouse_delta_input)
        ...
        self._control_panel._keyboard_widget.all_assignments_applied.connect(self._on_all_keyboard_assignments_applied)
    ...
    def _create_pettingzoo_env(
        self, task: str, seed: int | None, initial_state: Optional[str] = None
    ) -> Any:
    ...
    def _on_reset_pettingzoo_multiagent(self, seed: int | None, config: "OperatorConfig") -> None:
    ...
    def _on_mouse_delta_input(self, operator_id: str, dx: int, dy: int) -> None:
        """Handle mouse delta input from an operator container.

        Used for native speed control in Malmo/ViZDoom.
        """
        # If this is the main session (Single Agent tab or active session):
        if self._session and hasattr(self._session, "_interaction"):
            interaction = self._session._interaction
            if interaction and hasattr(interaction, "handle_native_mouse"):
                # Forward to the session's interaction controller
                interaction.handle_native_mouse(dx, dy)
    ...
    def _on_reset_parallel_multiagent(self, seed: int | None, config: "OperatorConfig") -> None:
    ...
    def _create_parallel_multiagent_env(
        self, env_name: str, task: str, seed: int | None, config: "OperatorConfig | None" = None,
    ) -> Any:
    ...
    def _resolve_agent_colors(
        ...
        """Build agent_id -> color_name from operator config.

        Reads agent_color from each worker's settings and validates it
        against COLOR_PALETTE. Falls back to the default
        ...
        try:
            ...
            if env_name in ("babyai", "minigrid"):
                ...
                try:
                    ...
                    game_file = str(game_files[(seed or 0) % len(game_files)])
                ...
                try:
                    ...
                    try:
                        rgb_frame = env.render(highlight=True)  # type: ignore[call-arg]
                    except TypeError:
                        rgb_frame = env.render()
                    env.close()
    ...
    def _on_session_initialized(self, game_id: str, mode: str, _step: object) -> None:
        ...
        if step_info:
            ...
            if num_agents and isinstance(num_agents, int) and num_agents > 1:
                ...
                # Auto-assign keyboards and launch workers immediately
                self._control_panel._keyboard_widget.auto_assign_and_apply()
        ...
        self._configure_mouse_capture()  # Configure environment-specific mouse capture
    ...
    def _configure_mouse_capture(self) -> None:
        ...
        Each loader returns True if it claimed the game; if none claim it,
        mouse capture is disabled so stale callbacks from a previous game
        are not left active.
        """
        claimed = self._vizdoom_env_loader.configure_mouse_capture(self._session)
        claimed = self._malmo_env_loader.configure_mouse_capture(self._session) or claimed
        claimed = self._smac_camera_loader.configure_mouse_capture(self._session) or claimed
        if not claimed:
    ...
    def _on_all_keyboard_assignments_applied(self, assignments: dict) -> None:
        """Handle the all_assignments_applied signal (fires ONCE with all assignments).

        This is the proper entry point for multi-agent keyboard setup.
        Launches one HumanKeyboardRuntime subprocess per keyboard.
        """
        if not assignments:
            return

        adapter = getattr(self._session, "_adapter", None)
        num_agents = getattr(adapter, "num_agents", getattr(adapter, "expected_agent_count", 1)) if adapter else 1
        if num_agents <= 1:
            num_agents = self._human_input._num_agents

        if num_agents <= 1:
            # Single-agent: use old evdev path
            self._keyboard_worker_bridge.stop()
            self._human_input.set_enabled(True)
            self._human_input.setup_evdev_keyboards(assignments)
            return

        # Multi-agent: launch subprocess workers
        ...
        self.log_constant(
            LOG_KEYBOARD_EVDEV_SETUP_START,
            message=f"Launching keyboard workers for {len(assignments)} keyboards (multi-agent)",
            extra={"num_keyboards": len(assignments), "assignments": assignments},
        )

        # Determine environment name for the key resolver
        env_name = "multigrid"
        game_id = self._session._game_id
        if game_id is not None:
            ...
            if family == EnvironmentFamily.MALMOENV:
                env_name = "malmo"
            elif family == EnvironmentFamily.MINIGRID:
                env_name = "minigrid"
            elif family == EnvironmentFamily.MELTINGPOT:
                env_name = "meltingpot"
            elif family == EnvironmentFamily.MOSAIC_MULTIGRID:
                env_name = "mosaic_multigrid"

        # For Malmo, extract the action list from the adapter
        action_list = None
        if env_name == "malmo" and adapter is not None:
            agents = getattr(adapter, "_agents", [])
            if agents and hasattr(agents[0], "action_space"):
                action_list = list(getattr(agents[0].action_space, "actions", []))

        # Disable ALL main-thread keyboard input before launching workers.
        # The subprocess workers own the keyboards exclusively.
        self._human_input.set_enabled(False)
        self._human_input.stop_evdev_monitoring()
        self._human_input.clear_evdev_keyboards()
        self._session._stop_idle_tick()

        success = self._keyboard_worker_bridge.start(
            assignments, env_name=env_name, action_list=action_list
        )

        if success:
            self.log_constant(
                LOG_KEYBOARD_EVDEV_SETUP_SUCCESS,
                message=f"Keyboard workers running for {len(assignments)} keyboards",
                extra={"num_keyboards": len(assignments)},
            )
        else:
            self.log_constant(
                LOG_KEYBOARD_EVDEV_SETUP_FAILED,
                message="Keyboard worker launch failed",
                extra={"assignments": assignments},
            )

    def _on_keyboard_assignment_changed(self, device_path: str, agent_id: object) -> None:
        """Handle per-keyboard assignment changes (single-agent only).

        For multi-agent, ``all_assignments_applied`` handles setup instead.
        This handler is only used for the single-agent evdev path.
        """
        # Multi-agent is handled by _on_all_keyboard_assignments_applied
        adapter = getattr(self._session, "_adapter", None)
        num_agents = getattr(adapter, "num_agents", getattr(adapter, "expected_agent_count", 1)) if adapter else 1
        if num_agents <= 1:
            num_agents = self._human_input._num_agents
        if num_agents > 1:
            return  # skip, let all_assignments_applied handle it
        ...
        # --- Single-agent: old evdev path on main thread ---
        if num_agents <= 1:
            self.log_constant(
                LOG_KEYBOARD_EVDEV_SETUP_START,
                message=f"Setting up evdev for {len(assignments)} keyboards (single-agent)",
                extra={"num_keyboards": len(assignments), "assignments": assignments},
            )

            # Stop any running subprocess workers and re-enable main-thread input
            self._keyboard_worker_bridge.stop()
            self._human_input.set_enabled(True)
        ...
        # --- Multi-agent: subprocess workers ---

        # Guard: skip if the bridge is already running with the same
        # assignments.  The assignment_changed signal fires once per
        # keyboard, so this handler is called N times in quick succession.
        if self._keyboard_worker_bridge.is_active:
            return
        ...
        if env_name == "malmo":
            ...
            if adapter is not None:
                ...
                if agents and hasattr(agents[0], "action_space"):
                    action_list = list(
                        getattr(agents[0].action_space, "actions", [])
                    )

        # Stop the old main-thread evdev monitor BEFORE launching
        # subprocess workers.  Both would read from the same /dev/input
        # device files, and the old monitor would steal events.
        self._human_input.stop_evdev_monitoring()
        self._human_input.clear_evdev_keyboards()
        ...
        if not success:
            self.log_constant(
                LOG_KEYBOARD_EVDEV_SETUP_FAILED,
                message="Keyboard worker launch failed - check permissions",
                extra={"assignments": assignments},
            )
            return
    ...
    def _on_keyboard_worker_actions_ready(self, actions: list) -> None:
        """Handle complete action list from keyboard worker subprocesses.

        Called when all keyboard workers have returned actions for the
        current round.  Steps the environment through the session's
        standard pipeline (rewards, replay, telemetry preserved), then
        tells the bridge to start the next round of action collection.
        """
        if not self._session._game_started:
            return
        self._session.perform_human_action(actions, key_label="multi_agent")
        # Start the next round: send select_action to all workers again
