...
class MainWindow(QtWidgets.QMainWindow, LogConstantMixin):
    ...
    def _on_start_game(self) -> None:
        ...
        # Discovers devices, pairs by USB port, assigns to agents, launches workers.
    ...
    def _on_keyboard_assignment_changed(self, device_path: str, agent_id: object) -> None:
        ...
        """
        # When bridge workers own the devices, skip old main-thread evdev
        if self._keyboard_worker_bridge.is_active:
            _LOGGER.debug(
                "Skipping main-thread evdev setup: bridge workers are active"
            )
            return
    ...
    def _on_operator_keyboard_assignment_changed(self, device_path: str, agent_id: object) -> None:
        ...
        if self._keyboard_worker_bridge.is_active:
            _LOGGER.debug(
                "Skipping main-thread evdev setup (Operators tab): bridge workers active"
            )
            return
    ...
    def _auto_launch_keyboard_workers(self) -> None:
        ...
        # Update keyboard widget to show assignments (informational)
        if num_agents > 1 and self._control_panel._keyboard_widget.isVisible():
            self._control_panel._keyboard_widget.set_available_agents(agent_names)
    ...
    def _on_all_keyboard_assignments_applied(self, assignments: dict) -> None:
        """Launch keyboard worker subprocesses when all assignments are applied.
        ...
        # Stop old main-thread evdev to release device fds before subprocesses open them
        if self._human_input._use_evdev:
            _LOGGER.info("Stopping main-thread evdev before launching bridge workers")
            self._human_input.stop_evdev_monitoring()
        ...
        if game_id is not None:
            gid_str = str(game_id).lower()
            if "malmo" in gid_str or "minecraft" in gid_str:
                env_name = "malmo"
            elif "vizdoom" in gid_str:
                env_name = "vizdoom"

        self.log_constant(
            LOG_HUMAN_CONTROL_BRIDGE_START,
            message=f"Starting keyboard workers: {len(assignments)} agents, env={env_name}",
            extra={"assignments": assignments, "env_name": env_name},
        )
        ...
        if success:
            self._status_bar.showMessage(
                f"Keyboard workers running for {len(assignments)} agent(s)",
                5000,
            )
        else:
            self._status_bar.showMessage(
